...
public class ProductProvider extends ContentProvider {
    ...
    private Uri insertProduct(Uri uri, ContentValues values) {
        ...
        if (name == null) {
            throw new IllegalArgumentException("Uniform requires a name");
        }
        ...
        if (quantity != null && quantity < 0) {
            throw new IllegalArgumentException("Uniform requires a valid quantity");
        }

        // Check that the name is not null
        String supplierName = values.getAsString(ProductContract.ProductEntry.COLUMN_PRODUCT_SUPPLIER_NAME);
        if (supplierName == null) {
            throw new IllegalArgumentException("Uniform requires a supplier's name");
        }

        // Check that the name is not null
        String supplierEmail = values.getAsString(ProductContract.ProductEntry.COLUMN_PRODUCT_SUPPLIER_EMAIL);
        if (supplierEmail == null) {
            throw new IllegalArgumentException("Uniform requires a supplier's email address");
        }
